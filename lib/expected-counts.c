...
void
set_rule_expectations(grammar g, FLOAT *rule_counts)
{
...
// Do a single IO thing and save the unnormalised expectations in the grammar.
FLOAT
inside_outside_single(grammar g, const si_t si, FILE *yieldfp, 
         FILE *tracefp, FILE *summaryfp, int debuglevel,
         int maxsentlen, int minits, int maxits,
         FLOAT stoptol, FLOAT minruleprob,
         FLOAT jitter, int normalise, FLOAT wordscale,
         ...
         int weighted_yields_flag)
{
  ...
  if (tracefp && debuglevel > 10000)
    ...
    set_rule_expectations(g, rule_counts);
    ...
    if (summaryfp && debuglevel >= 1000) {
      fprintf(summaryfp, "%d",(int) nrules);
      fflush(summaryfp);
    }
  ...
  return(sum_neglog_prob0/(log(2)*sum_yieldweights));
}
	       ...
	       int weighted_yields_flag)
{
  ...
  if (summaryfp && debuglevel >= 1000) {
    fprintf(summaryfp, "#It\ttemp.\tnrules\t-logP\tbits/token\n%d\t%g\t%d", 
  ...
  while (1) {
    ...
    // make maxits a hard boundary even if we have zeroed rules.
    if (maxits > 0 && iteration >= maxits) break;

    if (nrules==nrules0 && iteration >= minits &&
