...
class NeuralLearner:
    ...
    def select_anchors_divergence_ordered(self, max_terminals=300,
        ...
        # Fit noise as a function of frequency: noise = floor + c / sqrt(min_freq).
        # This captures two noise sources:
        #   - Data noise: scales as 1/sqrt(freq) (fewer contexts = noisier)
        #   - Model noise: irreducible floor from training stochasticity
        # We collect (min_freq, noise) for all pairs and fit via least squares.
        freq_noise_data = []
        for (a, b), n_val in noise.items():
            if a == '<S>' or b == '<S>':
                continue
            fa = self.word_counts.get(a, 0)
            fb = self.word_counts.get(b, 0)
            min_f = min(fa, fb)
            if min_f > 0:
        ...
        if len(freq_noise_data) > 10:
            ...
            # Least squares: noise = floor + c * (1/sqrt(freq))
            ...
            noise_floor_const = max(coeffs[0], 0.0)  # non-negative floor
            noise_freq_coeff = max(coeffs[1], 0.0)   # non-negative scaling
        else:
            noise_floor_const = float(np.median(
                [v for v in noise.values() if v > 0])) if noise else 0.0
            noise_freq_coeff = 0.0

        def noise_model(freq_a, freq_b):
            """Predicted noise floor for a pair given word frequencies."""
            min_f = max(min(freq_a, freq_b), 1)
            return noise_floor_const + noise_freq_coeff / math.sqrt(min_f)

        if verbose:
            print(f"  Noise model: {noise_floor_const:.3f} + "
                  f"{noise_freq_coeff:.3f} / sqrt(min_freq)")
            # Show a few example predictions
            for f in [20, 100, 1000, 10000]:
                print(f"    freq={f:>6}: noise_floor = "
                      f"{noise_model(f, f):.3f}")

        # 4b. Add S as a virtual anchor.
        # S's context distribution is a point mass on the boundary
        # (empty) context.  In CNF, length-1 sentences come only from
        # S -> w, so length1_counts[w] / word_counts[w] gives the
        # fraction of w's occurrences generated by S.
        #
        # D(S||w) = -log(s_frac) where s_frac = len1/count.
        # D(w||S) = infinity (w has non-boundary contexts where P_S=0).
        # We store a large finite sentinel for D(w||S) so that
        # corrected_distance(w, S) = D(S||w) - noise, which
        # naturally prevents S-heavy words from being selected.
        D_WS_SENTINEL = 1e6

        # Precompute split-half length-1 and word counts for noise
        split_count_A = Counter()
        split_len1_A = Counter()
        for s in self._split_sentences_A:
            for w in s:
                split_count_A[w] += 1
            if len(s) == 1:
                split_len1_A[s[0]] += 1

        split_count_B = Counter()
        split_len1_B = Counter()
        for s in self._split_sentences_B:
            for w in s:
                split_count_B[w] += 1
            if len(s) == 1:
                split_len1_B[s[0]] += 1
        ...
        n_s_entries = 0
        for w in candidates:
            count_w = self.word_counts.get(w, 0)
            count_1 = self.length1_counts.get(w, 0)

            if count_w > 0 and count_1 > 0:
                s_frac = count_1 / count_w
                div_full[('<S>', w)] = -math.log(s_frac)
                n_s_entries += 1

                # Split-half noise for D(S||w)
                cA = split_count_A.get(w, 0)
                c1A = split_len1_A.get(w, 0)
                cB = split_count_B.get(w, 0)
                c1B = split_len1_B.get(w, 0)
                d_sA = -math.log(c1A / cA) if cA > 0 and c1A > 0 else 1e6
                d_sB = -math.log(c1B / cB) if cB > 0 and c1B > 0 else 1e6
                if d_sA < 1e5 and d_sB < 1e5:
                    noise[('<S>', w)] = abs(d_sA - d_sB)
                else:
                    noise[('<S>', w)] = noise_model(1, count_w)
            else:
                div_full[('<S>', w)] = D_WS_SENTINEL

            div_full[(w, '<S>')] = D_WS_SENTINEL
            noise[(w, '<S>')] = 0.0  # irrelevant (sentinel direction)

        if verbose:
            print(f"  S divergences: {n_s_entries} terminals with "
                  f"length-1 occurrences")

        # 5. Corrected divergence function (DIRECTED, not symmetric).
        # D(s||w) - noise(s,w): the divergence from anchor s to
        # candidate w, corrected for estimation noise.
        #
        # For same-NT pairs this should be near zero (the anchor's
        # context distribution "contains" the candidate's).
        # For cross-NT pairs this is large (genuinely different).
        #
        # We use the frequency-dependent noise model as a floor.
        def corrected_divergence(s, w):
            """Noise-corrected D(s||w)."""
            d_sw = div_full.get((s, w), 0)
            fs = self.word_counts.get(s, 0) if s != '<S>' else 1
            fw = self.word_counts.get(w, 0) if w != '<S>' else 1
            floor = noise_model(fs, fw)
            n_sw = max(noise.get((s, w), floor), floor)
            return d_sw - n_sw
        ...
        def is_mixture(w, selected):
            ...
            The virtual S anchor is skipped here: D(w||S) = infinity
            (S is a point mass), so the ratio test is meaningless.
            S-filtering is handled by corrected_divergence instead.
            """
            for s in selected:
                if s == '<S>':
                    continue  # S handled by distance, not ratio
                d_sw = div_full.get((s, w), 0)  # anchor -> candidate
                d_ws = div_full.get((w, s), 0)  # candidate -> anchor
                fs = self.word_counts.get(s, 0)
                fw = self.word_counts.get(w, 0)
                default_n = noise_model(fs, fw)
                n_sw = noise.get((s, w), default_n)
                n_ws = noise.get((w, s), default_n)
        ...
        # 6. Initialize with S as virtual anchor + most frequent terminal.
        # S's context distribution is known a priori (boundary only),
        # so we don't need to find an S anchor empirically.  The
        # first real anchor is the most frequent terminal that is
        # not S-dominated (frequency correlates with being a genuine
        # anchor rather than a mixture word).
        non_s = [w for w in candidates if w != '<S>']
        if not non_s:
            self.anchors = []
            return []

        # Filter out S-dominated words from seed candidates
        # (> 50% of occurrences in length-1 sentences)
        seed_candidates = [
            w for w in non_s
            if (self.length1_counts.get(w, 0) /
                max(self.word_counts.get(w, 1), 1)) < 0.5
        ]
        if not seed_candidates:
            seed_candidates = non_s  # fallback

        best_start = max(seed_candidates,
                         key=lambda w: self.word_counts.get(w, 0))

        selected = ['<S>', best_start]  # virtual S + frequency seed
        remaining = set(non_s) - {best_start}
        rejected = set()
        consecutive_rejects = 0

        if verbose:
            print(f"\n  Init: {'<S>':>10} (virtual anchor)")
            freq = self.word_counts.get(best_start, 0)
            s_frac = (self.length1_counts.get(best_start, 0) /
                      max(self.word_counts.get(best_start, 1), 1))
            print(f"  Seed: {best_start:>10} (freq={freq:>7}, "
                  f"S_frac={s_frac:.3f})")

        # 7. Greedy furthest-point with asymmetry filter.
        #
        # Score for candidate w = min over selected anchors s of
        # corrected_divergence(s, w), i.e. the noise-corrected
        # D(nearest_anchor || w).  This uses the directed divergence
        # from anchor to candidate — the direction where same-NT
        # divergences are small and reliable.
        #
        # Stopping threshold: adaptive, estimated from the distribution
        # of corrected_divergence(s, w) across all (anchor, remaining)
        # pairs.  The bulk of these are same-NT (noise).  We estimate
        # σ from the MAD of the negative values, then threshold at
        # μ + τ·σ where τ controls the precision/recall tradeoff.
        # Default τ=8 gives very few false anchors.
        tau = 8.0  # precision/recall tradeoff parameter

        while remaining:
            best_w = None
            best_score = -float('inf')

            # Collect all (anchor→candidate) corrected divergences
            # for adaptive threshold estimation.
            all_scores = []

            for w in remaining:
                if w in rejected:
                    continue
                # Score = min D(s||w) - noise over selected anchors
                scores_w = [corrected_divergence(s, w) for s in selected]
                min_score = min(scores_w)
                all_scores.extend(scores_w)
                if min_score > best_score:
                    best_score = min_score
                    best_w = w

            # Hard stop: nothing left or all below zero
            if best_w is None or best_score <= 0:
                if verbose:
                    print(f"  Stop: best score = {best_score:.3f} <= 0")
                break

            # Adaptive threshold from the distribution of
            # corrected D(anchor||candidate) values.
            # The negative values are reliably same-NT (noise).
            neg_scores = [v for v in all_scores if v < 0]
            if len(neg_scores) > 10:
                mu = float(np.median(neg_scores))
                # MAD-based robust sigma (1.4826 converts MAD to
                # Gaussian sigma)
                mad = float(np.median([abs(v - mu)
                                       for v in neg_scores]))
                sigma = 1.4826 * mad
                threshold = mu + tau * sigma
            else:
                threshold = 0.0  # fallback: accept if > 0

            if best_score <= threshold:
                if verbose:
                    sigma_str = f"{sigma:.3f}" if len(neg_scores) > 10 \
                        else "n/a"
                    print(f"  Stop: best score = {best_score:.3f} <= "
                          f"threshold {threshold:.3f} "
                          f"(μ={mu:.3f}, σ={sigma_str}, τ={tau})")
                break
        ...
        # The first real anchor was selected without any non-S
        # reference set for mixture detection.  Now that we have the
        # full anchor set, check every real anchor against all OTHERS.
        # This catches cases where the first real anchor is itself a
        # shared word.  Skip the virtual '<S>' entry.
        ...
        while changed:
            ...
            for i in range(len(pruned)):
                if pruned[i] == '<S>':
                    continue  # never remove virtual S
        ...
        # 9. Strip virtual S and store results
        selected = [w for w in selected if w != '<S>']
        self.anchors = selected
        self.nonterminals = ['S'] + [f'NT_{w}' for w in selected]
        self.anchor2nt = {w: f'NT_{w}' for w in selected}
